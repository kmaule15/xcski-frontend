...
const SnowIndicator = ({ lat, lng }: { lat: number, lng: number }) => {
  ...
  return (
    <div>
      {weather === 'Snow' ? <img src={`${process.env.PUBLIC_URL}/SnowIcon.png`} alt="Snow Icon" className="snow-icon" /> : <img src={`${process.env.PUBLIC_URL}/noSnowIcon.png`} alt="No Snow Icon" className="snow-icon" />}
    </div>
  );
};
